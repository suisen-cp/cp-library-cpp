...
data:
  ...
  bundledCode: "#line 1 \"library/linear_algebra/matrix.hpp\"\n\n\n\n#include <cassert>\n\
    ...
    \ int m, T fill_value = T(0)) : dat(n, std::vector<T>(m, fill_value)) {}\n   \
    ...
    \ const Matrix<T>& B) { return A.dat == B.dat; }\n        friend bool operator!=(const\
    ...
    \ 0; j < m; ++j) for (int k = 0; k < l; ++k) res[i][k] += A.dat[i][j] * B.dat[j][k];\n\
    \            return res;\n        }\n        friend Matrix<T> operator*(const\
    \ T& val, Matrix<T> A) { A *= val; return A; }\n        friend Matrix<T> operator*(Matrix<T>\
    ...
    \        enum class Identity {\n            ADD, MUL\n        };\n        Matrix(int\
    \ n, Identity ident) : Matrix<T>::Matrix(n) {\n            if (ident == Identity::MUL)\
