#ifndef SUISEN_RANGE_CONTOUR_SUM_INVERTIBLE
...
namespace suisen {
    ...
    private:
        struct InternalCumulativeSum {
            InternalCumulativeSum(int n = 0) : _n(n), _dat(n + 1, _zero()) {}
            template <typename InputIterator>
            InternalCumulativeSum(InputIterator first, InputIterator last) : _n(last - first), _dat(_n + 1) {
                _dat[0] = _zero();
                for (int i = 0; i < _n; ++i) _dat[i + 1] = _add(_dat[i], *first++);
            }
            value_type sum(int l, int r) const {
                l = std::max(0, l), r = std::min(r, _n);
                return l < r ? _add(_dat[r], _neg(_dat[l])) : _zero();
            }
        private:
            int _n;
    ...
    public:
        RangeContourSumOnTree(int n = 0, const value_type& fill_value = _zero()) : RangeContourSumOnTree(std::vector<value_type>(n, fill_value)) {}
        RangeContourSumOnTree(const std::vector<value_type>& a) : _n(a.size()), _nodes(_n), _par(2 * _n, -1), _info(_n), _subtrees(2 * _n), _ord(_n) {
        ...
        value_type sum(int u, int dl, int dr) const {
            ...
            for (auto it = _info[u].begin(); it != it_end; ++it) {
                ...
                res = _add(res, _subtrees[std::exchange(v, _par[v])][it->child_index ^ 1].sum(ql, qr));
            }
            return res;
        }
