...
namespace suisen {
    ...
    struct RangeContourAggregationQueryOnTree {
        ...
        RangeContourAggregationQueryOnTree(int n, const T &fill_value) : RangeContourAggregationQueryOnTree(std::vector<T>(n, fill_value)) {}
        RangeContourAggregationQueryOnTree(const std::vector<T> &dat) : _n(dat.size()), _g(_n), _par(_n, -1), _removed(_n, false), _info(_n), _nodes(_n), _dat(dat) {
            _par.reserve(2 * _n);
