...
class RMQpm1 {
    ...
    static constexpr int e = is_min_query ? std::numeric_limits<int>::max() : std::numeric_limits<int>::min();
    static constexpr auto op = [](int x, int y) { return comp(x, y) ? x : y ; };
    ...
    public:
        RMQpm1(std::vector<int> &&x) : n(x.size()), m((n + SIZE - 1) >> LOG), a(std::move(x)), b(m, 0), tabl(build(), e, op) {}
