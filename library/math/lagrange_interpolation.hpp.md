...
data:
  ...
  bundledCode: "#line 1 \"library/math/lagrange_interpolation.hpp\"\n#include <vector>\n\
    ...
    \ - 1)};\n        }\n};\n} // namespace suisen\n\n\n#line 3 \"library/math/lagrange_interpolation.hpp\"\
  ...
  code: "#include <vector>\n#include \"library/math/fps.hpp\"\n\nnamespace suisen\
    \ {\ntemplate <typename T>\nT lagrange_interpolation(const std::vector<T> &ys,\
    \ const T t) {\n    const int n = ys.size();\n    T fac = 1;\n    for (int i =\
    \ 1; i < n; ++i) fac *= i;\n    std::vector<T> fci(n), suf(n);\n    fci[n - 1]\
    \ = T(1) / fac;\n    suf[n - 1] = 1;\n    for (int i = n - 1; i > 0; --i) {\n\
    \        fci[i - 1] = fci[i] * i;\n        suf[i - 1] = suf[i] * (t - i);\n  \
    \  }\n    T prf = 1, res = 0;\n    for (int i = 0; i < n; ++i) {\n        T val\
    \ = ys[i] * prf * suf[i] * fci[i] * fci[n - i - 1];\n        if ((n - 1 - i) &\
    \ 1) {\n            res -= val;\n        } else {\n            res += val;\n \
    \       }\n        prf *= t - i;\n    }\n    return res;\n}\n\ntemplate <typename\
  ...
  verifiedWith: []
documentation_of: library/math/lagrange_interpolation.hpp
layout: document
redirect_from:
- /library/library/math/lagrange_interpolation.hpp
- /library/library/math/lagrange_interpolation.hpp.html
