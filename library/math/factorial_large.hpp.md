...
data:
  ...
  bundledCode: "#line 1 \"library/math/factorial_large.hpp\"\n\n\n\n#include <cmath>\n\
    ...
    \ dp;\n    }\n} // namespace suisen\n\n\n#line 8 \"library/math/factorial_large.hpp\"\
    \n\nnamespace suisen {\n    template <typename mint>\n    struct FactorialLarge\
    \ {\n        static constexpr int _p = mint::mod();\n        static constexpr\
    \ int _b = 4000; // or sqrt(_p * log(_p)) ?\n        static constexpr int _q =\
    \ _p / _b;\n\n        FactorialLarge() {\n            // f/x = (x+1)(x+2)...(x+b)\n\
    \            FormalPowerSeries<mint> f = stirling_number1<FormalPowerSeries<mint>>(_b\
    \ + 1);\n            // 0,b,2b,...,(q-1)b\n            std::vector<mint> xs(_q);\n\
    \            for (int i = 0; i < _q; ++i) {\n                xs[i] = i * _b;\n\
    \            }\n            // _acc[i] = (ib+1)(ib+2)...(ib+b)\n            _acc\
    \ = multi_point_eval(f >> 1, xs);\n\n            // _acc[i]\n            // =\
    \ \\prod_{j=0}^{i-1} ((jb+1)(jb+2)...(jb+b))\n            // = (ib)!\n       \
    \     _acc.insert(_acc.begin(), 1);\n            for (int i = 1; i < static_cast<int>(_acc.size());\
    \ ++i) {\n                _acc[i] *= _acc[i - 1];\n            }\n        }\n\n\
  ...
  code: "#ifndef SUISEN_FACTORIAL_LARGE\n#define SUISEN_FACTORIAL_LARGE\n\n#include\
    ...
    \   template <typename mint>\n    struct FactorialLarge {\n        static constexpr\
    \ int _p = mint::mod();\n        static constexpr int _b = 4000; // or sqrt(_p\
    \ * log(_p)) ?\n        static constexpr int _q = _p / _b;\n\n        FactorialLarge()\
    \ {\n            // f/x = (x+1)(x+2)...(x+b)\n            FormalPowerSeries<mint>\
    \ f = stirling_number1<FormalPowerSeries<mint>>(_b + 1);\n            // 0,b,2b,...,(q-1)b\n\
    \            std::vector<mint> xs(_q);\n            for (int i = 0; i < _q; ++i)\
    \ {\n                xs[i] = i * _b;\n            }\n            // _acc[i] =\
