...
namespace suisen {
    ...
    struct SPS : public std::vector<T> {
        ...
        struct ZetaSPS : public std::vector<polynomial_type> {
            ...
            ZetaSPS() = default;
            ZetaSPS(const SPS<value_type>& f) : base_type::vector(subset_transform::ranked_zeta(f)), _d(f.cardinality()) {}
            ...
            ZetaSPS inv() const {
                return ZetaSPS(*this).inv_inplace();
            }
            ZetaSPS sqrt() const {
            ...
            ZetaSPS pow(long long k) {
                return ZetaSPS(*this).pow_inplace();
            }
            ZetaSPS& inv_inplace() {
