...
data:
  ...
  bundledCode: "#line 1 \"library/math/bigint.hpp\"\n\n\n\n#include <algorithm>\n\
    ...
    \               res.fix_sign();\n                return res;\n            } else\
    \ {\n                return lhs + (-rhs);\n            }\n        }\n        friend\
    \ BigInt operator*(const BigInt &lhs, const BigInt &rhs) { return karatsuba(lhs,\
    ...
    \        BigInt(std::vector<int32_t>::const_iterator start, std::vector<int32_t>::const_iterator\
    ...
    \        }\n\n        static BigInt add_unsigned(std::vector<int32_t> lhs, const\
    \ std::vector<int32_t> &rhs) {\n            if (lhs.size() < rhs.size()) return\
    \ add_unsigned(rhs, lhs);\n            for (size_t i = 0; i < rhs.size(); ++i)\
    \ lhs[i] += rhs[i];\n            BigInt res(lhs);\n            res.fix_carry();\n\
    \            return res;\n        }\n        static BigInt sub_unsigned(std::vector<int32_t>\
    \ lhs, const std::vector<int32_t> &rhs) {\n            assert(lhs.size() >= rhs.size());\n\
