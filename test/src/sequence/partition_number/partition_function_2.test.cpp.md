...
data:
  ...
  bundledCode: "#line 1 \"test/src/sequence/partition_number/partition_function_2.test.cpp\"\
    ...
    \n\n\n\n#line 5 \"library/sequence/partition_number.hpp\"\n\nnamespace suisen\
    \ {\n    template <typename FPSType>\n    std::vector<typename FPSType::value_type>\
    \ partition_number(int n) {\n        FPSType inv(n + 1);\n        inv[0] = 1;\n\
    \        for (int i = 1, k = 1; k <= n; k += 3 * i + 1, i++) {\n            if\
    \ (i & 1) --inv[k];\n            else ++inv[k];\n        }\n        for (int i\
    \ = 1, k = 2; k <= n; k += 3 * i + 2, i++) {\n            if (i & 1) --inv[k];\n\
    \            else ++inv[k];\n        }\n        inv.inv_inplace(n + 1), inv.resize(n\
    \ + 1);\n        return inv;\n    }\n} // namespace suisen\n\n\n#line 10 \"test/src/sequence/partition_number/partition_function_2.test.cpp\"\
...
layout: document
redirect_from:
- /verify/test/src/sequence/partition_number/partition_function_2.test.cpp
- /verify/test/src/sequence/partition_number/partition_function_2.test.cpp.html
