...
data:
  ...
  bundledCode: "#line 1 \"test/src/sequence/stirling_number1/abc247_h.test.cpp\"\n\
    ...
    \ : cnt) pq.push(suisen::stirling_number1_reversed<suisen::FPS<mint>>(v));\n \n\
  ...
  code: "#define PROBLEM \"https://atcoder.jp/contests/abc247/tasks/abc247_Ex\"\n\n\
    ...
    \n    std::vector<int> c(n);\n    for (auto &e : c) std::cin >> e, --e;\n \n \
    \   std::vector<int> cnt(n);\n    for (auto &e : c) ++cnt[e];\n    \n    auto\
    \ comp = [&](const std::vector<mint> &f, const std::vector<mint> &g) { return\
    \ f.size() > g.size(); };\n    std::priority_queue<std::vector<mint>, std::vector<std::vector<mint>>,\
